...
class Layer(Parameterized):
    ...
    def sample_from_conditional_with_input_prop(self, X, z=None, full_cov=False):
        samples, mean, var = self.sample_from_conditional(X, z=z, full_cov=full_cov)
...
class SVGP_Layer(Layer):
    ...
    def single_sample_conditional(self, X, full_cov=False):
    ...
    def conditional(self, X, full_cov=False):
        ...
        if full_cov is True:
            f = lambda a: self.single_sample_conditional(a, full_cov=full_cov)
            mean, var = tf.map_fn(f, X, dtype=(tf.float64, tf.float64))
            return tf.stack(mean), tf.stack(var)
        else:
            S, N, D = tf.shape(X)[0], tf.shape(X)[1], tf.shape(X)[2]
            X_flat = tf.reshape(X, [S * N, D])
            mean, var = self.single_sample_conditional(X_flat)
...
class GPMC_Layer(SVGP_Layer):
    ...
    def single_sample_conditional(self, Xnew, full_cov=False):
...
class GPR_Layer(SVGP_Layer):
    ...
    def set_data(self, X, Y, lik_variance):
        self._X = X
        self._Y = Y
        self._lik_variance = lik_variance

    def single_sample_conditional(self, X, full_cov=False):
        # needs set_data to be called first
        return self.build_predict(X, self._X, self._Y, self._lik_variance, full_cov=full_cov)

    def build_predict(self, Xnew, X, Y, lik_variance, full_cov=False):
        ## modified from GPR
    ...
    def build_likelihood(self, X, Y, lik_variance):
        ## modified from GPR
        K = self.kern.K(X) + tf.eye(tf.shape(X)[0], dtype=settings.float_type) * lik_variance
        L = tf.cholesky(K)
...
class SGPR_Layer(Layer):
    def __init__(self, kern, Z, num_outputs, mean_function, **kwargs):
        ...
        self.num_latent = num_outputs  # gplvm terminology

    def build_predict(self, Xnew,  X_mean, X_var, Y, variance, full_cov=False):
        return gplvm_build_predict(self, Xnew,  X_mean, X_var, Y, variance, full_cov=full_cov)

    def build_likelihood(self, X_mean, X_var, Y, variance):
        return gplvm_build_likelihood(self, X_mean, X_var, Y, variance)
