...
class DGP_Collapsed(DGP_Base):
    ...
    def propagate(self, X, full_cov=False, S=1, zs=None):
        Fs, ms, vs = self.inner_layers_propagate(X, full_cov=full_cov, zs=zs)
        self.layers[-1].set_data(ms[-1], vs[-1], self.Y, self.likelihood.likelihood.variance)
        return DGP_Base.propagate(self, X, full_cov=full_cov, S=S, zs=zs)

    @params_as_tensors
    def _build_likelihood(self):
        f_inner, _, _ = self.inner_layers_propagate(self.X, full_cov=False)
        return self.layers[-1].build_likelihood(f_inner, self.Y, self.likelihood.likelihood.variance)




class DGP_Heinonen(DGP_Base):
    """
