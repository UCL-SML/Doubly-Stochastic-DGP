...
with _settings.temp_settings(custom_config),\
    ...
    from doubly_stochastic_dgp.model_zoo import DGP_Heinonen, DGP_Damianou
    ...
    class TestDamianou(unittest.TestCase):
        def setUp(self):
            Ns, N, D_X, D_Y = 5, 6, 3, 2
        ...
        #     Ys = np.random.randn(Ns, D_Y)
        #
        #     kern = RBF(self.X.shape[1], lengthscales=0.5)
        #     mf = Zero()
        #     m_gpr = GPR(self.X, Y, kern, mean_function=mf)
        #     m_gpr.likelihood.variance = lik_var
        #     mean_gpr, var_gpr = m_gpr.predict_y(self.Xs)
        ...
        #     pred_mfull_gpr, pred_vfull_gpr = m_gpr.predict_f_full_cov(self.Xs)
        #
        #     kernels = [RBF(self.X.shape[1], lengthscales=0.5)]
        #     with defer_build():
        #         layers = init_layers_linear(self.X, Y, self.X, kernels,
        #                                     num_outputs=self.D_Y,
        #                                     mean_function=mf,
        #                                     white=False)
        #
        #
        #     m_dgp = DGP_Damianou(self.X, Y, lik, layers)
        #
        #     var_list = []
        #     for layer in m_dgp.layers:
        #         var_list.append([layer.q_mu, layer.q_sqrt])
        #     NatGradOptimizer(gamma=1).minimize(m_dgp, var_list=var_list, maxiter=1)
        #
        #     mean_dgp, var_dgp = m_dgp.predict_y(self.Xs, 1)
        #     test_lik_dgp = m_dgp.predict_density(self.Xs, Ys, 1)
        ...
        def test_vs_two_layer(self):
            ...
            kernels = [
                       RBF(self.X.shape[1], variance=1e-12, lengthscales=0.5),
                       RBF(self.X.shape[1], variance=1e-12, lengthscales=0.5),
                       RBF(self.X.shape[1], lengthscales=0.5)
                       ]
            with defer_build():
                layers = init_layers_linear(self.X, Y, self.X, kernels,
                                            num_outputs=self.D_Y,
            ...
            m_dgp.layers[1].q_X_sqrt = m_dgp.layers[1].q_X_sqrt.read_value() * 1e-12
            print(m_dgp)
            ...
            print(m_dgp.compute_log_likelihood())
            print(m_gpr.compute_log_likelihood())

            NatGradOptimizer(gamma=1).minimize(m_dgp, var_list=var_list, maxiter=1)
            print(m_dgp.compute_log_likelihood())

            # not stochastic
            assert m_gpr.compute_log_likelihood() == m_gpr.compute_log_likelihood()
